-- 056_audit_actor_type_fix.sql
-- Fix actor_type assignment for security.anomaly.detected in
-- rpc_admin_write_audit_event.
--
-- Root cause: 052_audit_taxonomy_sync.sql did not include
-- 'security.anomaly.detected' in the 'system' actor_type CASE block,
-- causing client-side anomaly writes (from AuditLogPage detectAnomalies
-- useEffect) to fall through to ELSE → 'admin'.
-- The audit-anomaly-sweep Edge Function (direct service-role insert) was
-- already correct; only the RPC path was affected.
